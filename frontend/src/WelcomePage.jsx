...
const WelcomePage = ({ showForm = true }) => {
  ...
  const checkActivation = async () => {
    try {
      ...
      if (data.data.evalStatus === "active" && isValidEvalToken(data.data.evalToken)) {
        stopPolling();
        navigate(`/evaluation/${data.data.evalToken.trim()}`);
        return;
      }
      ...
      if (data.data.evalStatus === "active" && !isValidEvalToken(data.data.evalToken)) {
        stopPolling();
        setStatusMessage(
          "Lien d'evaluation invalide. Merci de contacter le service RH.",
        );
        return;
      }
